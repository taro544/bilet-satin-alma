<?php
// Output buffering başlat ve hata raporlamayı kapat
ob_start();
error_reporting(0);

// Veritabanı bağlantısını manual yapalım (config.php session_start() yapıyor)
...
if (!$order) {
    while (ob_get_level()) ob_end_clean();
    die("Sipariş bulunamadı veya size ait değil");
}
...
// Tüm output buffer'ları temizle ve output'u durdur
while (ob_get_level()) {
    ob_end_clean();
}

// Headers temizle ve PDF headers ayarla
if (headers_sent()) {
    die('Headers already sent. Cannot generate PDF.');
}

// PDF'i çıktı olarak ver
$filename = 'Siparis_' . $order['id'] . '_' . date('Y-m-d') . '.pdf';

// Önce inline göstermeyi dene, sorun varsa download yap
try {
    $pdf->Output($filename, 'I'); // I = Inline (tarayıcıda göster)
} catch (Exception $e) {
    // Inline çalışmazsa dosya olarak indir
